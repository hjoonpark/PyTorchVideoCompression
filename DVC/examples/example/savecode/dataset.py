...
class DataSet(data.Dataset):
    def __init__(self, path="data/vimeo_septuplet/test.txt", im_height=256, im_width=256):
        self.image_input_list, self.image_ref_list = self.get_vimeo_v2()
    ...
    def get_vimeo_v2(self, rootdir="../../data/vimeo_septuplet_v1/vimeo_interp_test/"):
        ...
        for sf in subfolders:
            ...
            for sf2 in subfolders2:
                img_paths = glob.glob(os.path.join(sf2, "*.png"))
                for img_path in img_paths:
                    path = os.path.normpath(img_path)
                    paths = path.split(os.sep)
                    y = os.path.join(input_dir, paths[-3], paths[-2], paths[-1])
                    # refnumber = int(y[-5:-4])
                    refname = os.path.join(target_dir, paths[-3], paths[-2], paths[-1])
